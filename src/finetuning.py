...
class LoRA(nn.Module):
    def __init__(self, original_layer, r=4, alpha=32):
        ...
        out_features = original_layer.weight.size(0)
        in_features = original_layer.weight.size(1)
    ...
    def forward(self, x):
        ...
        lora_update = x.matmul(self.A).matmul(self.B) * self.scaling
...
def inject_lora_into_model(model, r=4, alpha=32, device='cpu'):
    ...
    for child_name, child_module in list(model.named_children()):
        ...
        if (
            ...
            and child_name.lower() in ["q", "k", "v", "o"]
        ):
